...
<style>
    ...
    .hf-ekran .hf-kpis { display:grid; grid-template-columns:repeat(5,minmax(0,1fr)); gap:12px; margin-bottom:16px; }
    ...
    @media (max-width: 1100px) { .hf-ekran .hf-kpis { grid-template-columns:repeat(3,minmax(0,1fr)); } }
    ...
    .dark .hf-ekran .hf-chip, .dark .hf-ekran .hf-card, .dark .hf-ekran .hf-section, .dark .hf-ekran .hf-tavsiye { background:var(--card); color:var(--text); }
...
<div class="hf-ekran">
    @unless ($hideTitle)
        <div class="hf-head">
            <div>
                ...
                <div class="hf-sub">Aylık karşılaştırma · haftalık raporların SLA özeti (ölçü birimleri karıştırılmaz)</div>
            </div>
    ...
    @else
        <div class="hf-head">
            <div class="hf-sub">Haftalık raporların SLA özeti. Adet, arıza ve tutanak tek toplamda toplanmaz.</div>
            <span class="hf-chip {{ ($screen['durum'] ?? '') === 'Tamamlandı' ? 'hf-ok' : (($screen['rapor_var'] ?? false) ? 'hf-warn' : 'hf-miss') }}">
                {{ $screen['durum'] ?? '—' }}
            </span>
        </div>
    ...
    @else
        <div class="hf-kpis">
            ...
            <div class="hf-card">
                <div class="hf-kpi-l">En zayıf hafta</div>
                <div class="hf-kpi-v" style="color:var(--danger)">
                    {{ isset($ozet['en_zayif']['oran']) ? '%'.$ozet['en_zayif']['oran'] : '—' }}
                </div>
                <div class="hf-kpi">{{ $ozet['en_zayif']['etiket'] ?? '' }}</div>
            </div>
            <div class="hf-card">
                <div class="hf-kpi-l">En güçlü hafta</div>
                <div class="hf-kpi-v" style="color:var(--ok)">
                    {{ isset($ozet['en_guclu']['oran']) ? '%'.$ozet['en_guclu']['oran'] : '—' }}
                </div>
                <div class="hf-kpi">{{ $ozet['en_guclu']['etiket'] ?? '' }}</div>
            </div>
            <div class="hf-card">
                <div class="hf-kpi-l">Son açık kalem</div>
        ...
        @if ($chartN > 0)
            <div class="hf-section">
                ...
                <div class="hf-hint">SLA = planı olan kalemlerde gerçekleşen / öngörülen. Çizgi %100 hedeftir. Farklı ölçü birimleri toplanmaz.</div>
                <div class="hf-legend">
                    <span><b style="background:#94a3b8"></b>SLA eşiği %100</span>
                    <span><b style="background:#2563eb"></b>Hafta SLA</span>
                </div>
                <div class="hf-chart-wrap">
                    <svg class="hf-chart" viewBox="0 0 {{ $svgW }} {{ $svgH }}" role="img" aria-label="Haftalık SLA gerçekleşme oranı">
                        ...
                        <line x1="{{ $padL }}" y1="{{ $padT }}" x2="{{ $svgW - $padR }}" y2="{{ $padT }}" stroke="#86efac" stroke-width="1" stroke-dasharray="4 4"/>
                        @foreach ($chartCats as $i => $etiket)
                            @php
                                ...
                                $barW = min(28, $slotW * 0.45);
                                $realH = ($real / $localMax) * $plotH;
                                $realX = $cx - ($barW / 2);
                                $realY = $padT + $plotH - $realH;
                                $realColor = ($real + 0.0001 >= $plan) ? '#16a34a' : (($real < 70) ? '#dc2626' : '#2563eb');
                            @endphp
                            <rect x="{{ $realX }}" y="{{ $realY }}" width="{{ $barW }}" height="{{ max(2, $realH) }}" rx="3" fill="{{ $realColor }}"/>
                            <text x="{{ $cx }}" y="{{ $realY - 4 }}" text-anchor="middle" font-size="11" font-weight="700" fill="#1d4ed8">{{ ($haftalar[$i]['rapor_var'] ?? false) ? '%'.$fmt($real) : '—' }}</text>
                            <text x="{{ $cx }}" y="{{ $svgH - 14 }}" text-anchor="middle" font-size="11" font-weight="600" fill="#374151">{{ $etiket }}</text>
                        @endforeach
                    </svg>
                </div>
            </div>
        ...
        <div class="hf-section">
            <h3>Hafta özeti</h3>
            <div class="hf-hint">Her satır bir haftalık faaliyet raporudur.</div>
            <table>
                <thead>
                    <tr>
                        <th>Hafta</th>
        ...
        <div class="hf-section">
            ...
            <div class="hf-hint">Hücreler gerçekleşen/öngörülen. Ay sütunu girilen haftaları toplar. Katalogdaki KPI / SLA metni kalemin yanındadır.</div>
                    ...
                    <tbody>
                        @foreach ($rows as $row)
                            <tr class="t-{{ $row['tone'] ?? 'neutral' }}">
                                <td>{{ $row['kod'] }}</td>
                                <td>
                                    <strong>{{ $row['kalem'] }}</strong>
                                    <div class="hf-kpi">{{ $row['aile'] !== '' ? $row['aile'] : '' }}</div>
                                </td>
                                ...
                                <td class="num">
                                    <span class="hf-badge b-{{ $row['tone'] ?? 'neutral' }}">{{ $row['sla'] }}</span>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
